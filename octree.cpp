...
void Octree::Build( Mesh* triMesh )
{
    ...
    // Calculate mesh bounds and centroids
    aabb meshBounds;
    for (int i = 0; i < mesh->triCount; i++) {
...
void Octree::Subdivide( uint nodeIdx, uint depth, const float3& pMin, const float3& pMax) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        child.firstChildIdx = node.firstChildIdx + offsets[i];
...
void Octree::Intersect(Ray& ray, uint instanceIdx) {
    ...
    while (true) {
        if (node->isLeaf()) {
            for (uint i = 0; i < node->triCount; i++) {
                uint instPrim = (instanceIdx << 20) + triIdx[node->firstChildIdx + i];
                IntersectTri(ray, mesh->tri[instPrim & 0xfffff], instPrim);
            }
            if (stackPtr == 0) break;
            else {
                node = stack[--stackPtr];
